...
\cref{fig:thesis_generic_sgf_dry_ground} is more suitable to show the
versatility of the own implementation.
Considered are the generic scalar Green's functions, both for the \ac{TM} and
the \ac{TE} case, as specified by \cref{def:generic_scalar_gf}.
On the left-hand side the absolute value of the respective case is visualized,
while on the right-hand side the direct term is extracted and the real part
of the isolated reflected contribution to the Green's function is shown.
Both from the absolute value consideration of the complete Green's function, and
from the reflection-only consideration, significant differences between
can be observed which originate from the different reflection coefficients
in the respective \acp{TLGF}.
...
\cref{fig:dgf_G_EJ_full} eventually visualizes all nine components of
$\dyadicGF{EJ}$ for a source at $\rsrc = \lambda_0 \uv{z}$ over dry ground
in the $y = 0$ cut plane.
It is observed that those components that are theoretically expected to vanish
really do vanish in the numerical solution.
...
\cref{fig:dcim_dry_ground} shows a \ac{DCIM} example for the generic scalar
Green's functions.
The relative error is computed against a solution by numerical integration.
From the plot it is observed that a proper implementation of three-level
\ac{DCIM} indeed can yield results of reasonable accuracy.
