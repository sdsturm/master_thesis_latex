...
The representation $\uv{x}x + \uv{y}y + \uv{z}z$ of a vector $\pvec{r} \in \R^3$
in Cartesian coordinates relates to the common curvilinear systems according to
...
from vectors, \eg, $\vec{v}$, by the use of upper and lower case symbols,
respectively.
...
\begin{definition}[Fourier transform with respect to time]
	...
	\begin{equation}\label{eq:fourier_transfrom_temporal_forward}
		\fourierTransformForward{f \left(t\right)} \coloneqq
		...
		\, ,
	\end{equation}
	where $\omega$ is called \emph{angular frequency}.
\end{definition}
...
\begin{lemma}[Inverse Fourier tansform with respect to time]
	...
	\begin{equation}\label{eq:fourier_transform_temporal_backward}
		\fourierTransformBackward{\tilde{f} \left(\omega\right)} = 
		f \left( t \right) =
		\frac{1}{2\pi}
...
\begin{remark}[Time convention]\label{rem:time_convention}
	...
	or \acp{PDE} with respect to time, then
	...
	literature are adapted to the $\exp{\im \omega t}$ convention\footnote{
		Some major references to the present thesis, \eg, the ones by
		Sommerfeld, use the $\exp{-\mathrm{i} \omega t}$ time convention.
		The conversion to $\exp{\im \omega t}$ is done by the formal replaced
...
According tho this, the spatial counterpart to
...
\begin{equation}\label{eq:fourier_transform_3space_backward}
	\fourierTransformBackward{\tilde{f} \left( \pvec{k} \right)} = 
	f \left( \pvec{r} \right) = 
...
\begin{equation}\label{eq:fourier_transform_4d_inverse}
	\fourierTransformBackward{ \tilde{f} \left(\pvec{k}, \omega\right) } = 
	f \left( \robs, t \right) = 
...
pair of spatial transforms than it is in the temporal pair.
...
the homogeneous scalar wave equation \cite[p.~480]{Griffiths2011}
...
\end{equation}
in the \enquote{physical} $\left(\pvec{r}, t\right)$-domain.
...
Cartesian direction.
We shall assume this direction to be the one of $\uv{z}$.
Doing so, the expansion plane becomes the $xy$ plane of the
...
\begin{equation}\label{eq:fourier_transform_2space_forward}
	\tilde{f} \left( \pvec{k}_\rho, z\right) = 
	\iint\limits_{\R^2} \dd \pvec{\rho} \,
	\exp{\im \pvec{k}_\rho \cdot \pvec{\rho}}
	f \left( \pvec{r} \right)
\end{equation}
and
\begin{equation}\label{eq:fourier_transform_2space_backward}
	...
	\iint\limits_{\R^2} \dd \pvec{k}_\rho \,
	\exp{-\im \pvec{k}_\rho \cdot \pvec{\rho}}
	\tilde{f} \left( \pvec{k}_\rho, z \right)
	\, ,
\end{equation}
where $\pvec{\rho} = \uv{x}x + \uv{y}y$ and
$\pvec{k}_\rho = \uv{x}k_x + \uv{y}k_y$ are the respective components of
$\pvec{r}$ and $\pvec{k}$ transverse to $\uv{z}$.
Note that $z = \uv{z} \cdot \pvec{r}$ is unaffected by
\eqref{eq:fourier_transform_2space_forward} so it is sufficient to consider only
$f = f\left(\pvec{\rho}\right)$.

We shall further assume that both functions $f$ and $\tilde{f}$ possess---or
...
\eqref{eq:fourier_transform_2space_backward} can be reduced to
integrals over the radial coordinate only.
The resulting integral transforms are referred to as Hankel
transforms or, synonymously, Fourier-Bessel transforms in mathematical
context \cite[pp.~227]{Davies2002} \cite[p.~1086]{vanBladel2007}.
Since the Hankel transform is essentially a special case of a two-dimensional
...
\begin{corollary}[Hankel transform and inverse Hankel transform]
	...
	Let $f: \R^3 \mapsto \C$ be a function of azimuthal symmetry
	$f = f\left(\rho\right)$ and assume the integral 
	\eqref{eq:fourier_transform_2space_forward} exists for $f$.
...
\end{corollary}
\begin{proof}
	Express both $f$ and $\tilde{f}$ in planer polar coordinates, \ie,
	\begin{equation}
		f \left(\pvec{\rho}\right) = f \left(\rho, \varphi\right)
		\, ,
	\end{equation}
	\begin{equation}
		\tilde{f} \left(\pvec{k}_\rho\right)  =
		\tilde{f} \left(k_\rho, k_\varphi\right)
		\, .
	\end{equation}
	Employing their $2\pi$-periodicity in $\varphi$ and $k_\varphi$, both
	functions can be expanded into Fourier series representations
	\begin{equation}\label{eq:hankel_proof_00}
		f \left(\rho,\varphi\right) =
		\sum_{m \in \Z} f_m \left(\rho\right) \exp{\im m \varphi}
	\end{equation}
	and
	\begin{equation}
		\tilde{f} \left(k_\rho, k_\varphi\right) = 
		\sum_{n \in \Z} \tilde{f}_n \left(k_\rho\right) \exp{\im n k_\varphi}
		\, ,
	\end{equation}
	respectively, where the expansion coefficients are determined by
	\begin{equation}
		f_m\left(\rho\right) = \frac{1}{2\pi}
		\int\limits_{0}^{2\pi} \dd \varphi \,
		f \left(\rho, \varphi\right) \exp{-\im m \varphi}
	\end{equation}
	and
	\begin{equation}\label{eq:proof_hankel_01}
		\tilde{f}_n \left(k_\rho\right) = \frac{1}{2\pi}
		\int\limits_{0}^{2\pi} \dd k_\varphi \,
		\tilde{f} \left(k_\rho, k_\varphi\right) \exp{-\im n k_\varphi}
		\, .
	\end{equation}
	The 2D planar integral in \eqref{eq:fourier_transform_2space_forward}
	can be written as
	\eqref{eq:fourier_transform_2space_forward}
	\begin{equation}\label{eq:hankel_proof_02}
	\tilde{f} \left( k_\rho, k_\varphi \right) = 
	\int\limits_{0}^{\infty} \dd \rho \, \rho
	\int\limits_{0}^{2\pi} \dd \varphi \,
	\exp{\im k_\rho \rho \cos \left(\varphi - k_\varphi\right)}
	f \left( \rho, \varphi \right)
	\, .
	\end{equation}
	Upon substituting \eqref{eq:hankel_proof_02} into \eqref{eq:proof_hankel_01}
	and using \eqref{eq:hankel_proof_00} to represent
	$f \left(\rho,\varphi\right)$ we obtain
	\begin{equation}\label{eq:hankel_proof_03}
		\tilde{f}_n \left(k_\rho\right)  = \frac{1}{2\pi}
		\int\limits_{0}^{2\pi} \dd k_\varphi
		\exp{-\im n k_\varphi}
		\int\limits_{0}^{\infty} \dd \rho \, \rho
		...
		\, .
	\end{equation}
	Due to the orthogonality of the exponential functions all terms ob the sum
	but the $n$-th vanish and \eqref{eq:hankel_proof_02} is reduced to
	\begin{equation}
		\tilde{f}_n \left(k_\rho\right)  = 
		\int\limits_{0}^{\infty} \dd \rho \,
		\rho f_m \left(\rho\right)
		\int\limits_{0}^{2\pi} \dd \alpha \,
		\exp{\im \left(n \alpha + k_\rho \rho \cos \alpha\right)}
		\, ,
	\end{equation}
	where $\alpha \coloneqq \varphi - k_\varphi$ is used. Finally, the
	integral representation of the Bessel function $\besselj{n}$
	\cite[(10.9.2)]{Olver2010} can be used to obtain
	\eqref{eq:hankel_transform_forward}. The inverse transform
	\eqref{eq:hankel_transform_inverse} can be derived analogously.
\end{proof}

\fxnote{Explain what the procedure of the proof means}
...
However, in this context only inverse Hankel transforms of orders
...
\begin{definition}[Sommerfeld integrals]	
	Inverse Hankel transforms for orders $\nu \in \left\{0, 1, 2\right\}$
	are called \emph{Sommerfeld integrals} and are denoted by
	\begin{equation}\label{eq:si_sommerfeld_integration_path}
		\sommerfeldIntegral{\nu}{\tilde{f} \left( k_\rho \right)} \coloneqq 
		\frac{1}{2\pi}
		...
		\, .
	\end{equation}
	The particular path of integration along the positive real axis is called
	\emph{\acf{SIP}}.
\end{definition}
...
\begin{corollary}[Extended integration path]
	...
	\begin{equation}\label{eq:si_extended_integration_path}
		...
		\tilde{f} \left(k_\rho\right) \, .
	\end{equation}
	The path of integration over the whole real axis is called \acf{EIP}.
\end{corollary}
The proof of the equivalence of \eqref{eq:si_sommerfeld_integration_path} and
\eqref{eq:si_extended_integration_path}, which has proven so fruitful in the
early days of the study of wave propagation, is nowadays a classic textbook
