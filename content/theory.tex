...
Substituting these into \eqref{eq:scalar_free_space_gf}, consideration of the
absolute value
...
order to allow large magnitude $R$.

There are, however, also arguments for a definition of $R$ with non-negative
real part \cite{Hansen2009,Hansen2013}.
Intuitively, such a definition is closer to the notion of \cref{def:metric},
by which a distance is always a non-negative real number.
A straightforward approach to define complex distance could be
\begin{equation}
	R = \sqrt{\left(\rdiff\right) \cdot \overline{\left(\rdiff\right)}}
	\, ,
\end{equation}
% where, however a decision must be made regarding the s.
The effect of the two particular choices $\real{R} \ge 0$ and $\imag{R} \leq 0$
are illustrated in 



Both choices are, in principle, possible with 
\begin{definition}[Complex distance]
	Let $\robs \in \R^3$ and $\rsrc \in \C^3$ be two vectors and let the branch
	cut of the square root $\sqrt{z}$ for $z\in\C$ be defined along the
	non-positive real axis to $-\infty$.
	Then the \emph{complex distance} of $\robs$ from $\rsrc$ is defined by
	the double valued mapping
	\begin{equation}
		\cmplxlength{\rdiff} = 
		\sqrt{\left(\rdiff\right) \cdot \overline{\left(\rdiff\right)}}
	\end{equation}
	up to the choice of one of the two Riemann sheets.
\end{definition}
