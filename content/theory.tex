...
\end{equation}
in the \enquote{physical} domain $\left(\pvec{r}, t\right)$, where $c$ is
...
% For the moment we shall further assume that both functions $f$ and $\tilde{f}$
% possess---or can be coordinate-transformed to obtain---azimuthal
% symmetry, \ie, independence of $\varphi$ in \cref{def:cyl_coords}.
...
transform. A dual approach for the forward transform can be found
in the textbook on integral transforms by \textcite[p229]{Davies2002}}.
...
$k = k\left(z\right)$.
The sole functional dependency of the wavenumber on the Cartesian and
cylindrical  $z$-coordinate can be employed to reduce the governing Helmholtz
